...
class RSTnet(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=9,
        ...
        self.cbam_modules = nn.ModuleList([
            CBAM(in_planes=int(embed_dim * 2 ** i)) for i in range(self.num_layers)
        ])

        # Conv Encoder
        conv_dims = [embed_dim, embed_dim * 2, embed_dim * 4, embed_dim * 8]
        ...
        for i_layer in range(self.num_layers):
            decoder_stage_dim = int(embed_dim * 2 ** (self.num_layers - 1 - i_layer))
            if i_layer < self.num_layers - 1:
                self.rcasc_modules.append(RCASC(in_channels=decoder_stage_dim))

            if i_layer > 0:
                conv_skip_dim = conv_dims[self.num_layers - 1 - i_layer]
                self.fusion_convs.append(nn.Conv2d(conv_skip_dim, decoder_stage_dim, kernel_size=1))
            else:
                self.fusion_convs.append(nn.Identity())

            if i_layer == 0:
                layer_up = PatchExpand(input_resolution=(self.patches_resolution[0] // (2 ** (self.num_layers - 1)),
                                                          self.patches_resolution[1] // (2 ** (self.num_layers - 1))),
                                       dim=self.num_features, dim_scale=2, norm_layer=norm_layer)
            else:
                layer_up = BasicLayer_up(dim=decoder_stage_dim,
    ...
    def forward_decoder(self, x, swin_skips, conv_skips):
        ...
        for i, layer_up in enumerate(self.decoder_layers):
            if i > 0:
                swin_skip_1d = swin_skips[self.num_layers - i]
                conv_skip_2d = conv_skips[self.num_layers - 1 - i]

                B, L_swin, C_swin = swin_skip_1d.shape
                H_swin = self.patches_resolution[0] // (2 ** (self.num_layers - i))
                W_swin = self.patches_resolution[1] // (2 ** (self.num_layers - i))
                swin_skip_2d = swin_skip_1d.view(B, H_swin, W_swin, C_swin).permute(0, 3, 1, 2).contiguous()

                rcasc_swin_skip = self.rcasc_modules[i - 1](swin_skip_2d)
                fused_skip = rcasc_swin_skip + self.fusion_convs[i - 1](conv_skip_2d)

                B_x, L_x, C_x = x.shape
                H_x, W_x = H_swin, W_swin
    ...
    def forward(self, x):
        ...
        swin_bottleneck, swin_skip_features = self.forward_swin_encoder(x)
        bottleneck_output = self.conv_bottleneck(swin_bottleneck)
        decoder_output = self.forward_decoder(bottleneck_output, swin_skip_features, conv_features)
...
class RST_SwinUnet(nn.Module):
    def __init__(self, config, img_size=224, num_classes=9, zero_head=False, vis=False):
        ...
        self.rst_net.img_size = img_size
    ...
    def load_from(self, config):
        print("RSTnet does not support loading from pretrained Swin-T weights in this script.")
        pass
'''
